...
class JogoVelha {
    ...
    render() {
        ...
        var resultado = this.verificaVitoria();
        if (resultado == 'X' || resultado == 'O') {
            this.fim = true;
            console.log(`O jogador ${resultado} venceu!`);
        }
        
        //iterar os elementos no tabuleiro
        var velhaElemento = document.querySelectorAll('[data-id]');

        for (let i = 0; i < velhaElemento.length; i++) {
    ...
    verificaVitoria() {
        ...
        //deciml da sequencia de quem jogou X
        var valorX = parseInt(this.jogadas.map(value => value == 'X' ? 1 : 0).join(""), 2);

        //deciml da sequencia de quem jogou y
        var valorO = parseInt(this.jogadas.map(value => value == 'O' ? 1 : 0).join(""), 2);
